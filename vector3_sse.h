...
inline Vector3 add(const Vector3 a, const Vector3 b) { return _mm_add_ps(a, b); }
inline Vector3 sub(const Vector3 a, const Vector3 b) { return _mm_sub_ps(a, b); }
...
inline Vector3 mul(const Vector3 b, float a) { 
    __m128 tmp = _mm_load1_ps(&a);
    return _mm_mul_ps(tmp, b);
}

inline Vector3 div(const Vector3 a, float b) { 
...
inline float dot(const Vector3 a, const Vector3 b)     //copied from the internet, not verified
{ 
...
inline Vector3 cross(const Vector3 a, const Vector3 b)      //copied from the internet, not verified
{
  return _mm_sub_ps(
...
inline float length(const Vector3 a) { return sqrtf( dot(a,a) ); }
